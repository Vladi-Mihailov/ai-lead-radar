...
class OcrResult:
    ...
    владельца):
    - policyholder_full_name — из техпаспорта, ВСЕГДА целиком, как
      распознано (никогда не усекается). Если паспорт/права и техпаспорт
      распознаны и ФИО совпадает — то же самое ФИО, driver same_as-флаг
      True. Совпадение для driver_same_as_policyholder сравнивает ТОЛЬКО
      фамилию и имя (см. reader/ocr/service.py::_names_match) — разное или
      отсутствующее отчество (типично для водительских прав) не делает
      людей разными. Если фамилия или имя различаются — policyholder
      берётся из техпаспорта, driver_full_name — из паспорта/прав (тоже
      целиком, без усечения), driver_same_as_policyholder=False. Если
      техпаспортное ФИО не распознано — policyholder_full_name=None (ФИО
      одного паспорта не может заменить страхователя — это другая
      бизнес-роль).
