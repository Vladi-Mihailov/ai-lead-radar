...
def _derive_name_roles(
    *,
    ...
    паспорта/прав. Если оба ФИО распознаны и отличаются (см. _names_match —
    сравниваются только фамилия и имя, отчество игнорируется) — водитель
    отдельный (из паспорта/прав). Иначе (совпадают, или доступен только
    техпаспорт) — водитель = страхователь по умолчанию (~99% бизнес-случаев).
...
def _names_match(a: str, b: str) -> bool:
    """Только для определения driver_same_as_policyholder (см.
    _derive_name_roles) — сравниваются ТОЛЬКО фамилия и имя (первые два
    значимых слова после нормализации пробелов/casefold), отчество/третья
    часть ФИО игнорируется: разные документы (например, права) часто не
    содержат отчество, и совпадения фамилии+имени уже достаточно надёжно,
    чтобы считать это одним человеком. Само сравниваемое значение при этом
    НЕ усекается нигде, кроме этой проверки — policyholder_full_name/
    driver_full_name всегда хранят и показывают ФИО целиком, как распознано."""
    return _significant_name_parts(a) == _significant_name_parts(b)


def _significant_name_parts(value: str) -> tuple[str, ...]:
    """Фамилия + имя — первые два слова нормализованного ФИО (третье и
    далее — отчество и т.п., см. _names_match)."""
    return tuple(_normalize_name(value).split()[:2])
