...
структурная копия reader/turkey_bot_test/avrasya/live_session_registry.py::
...
lock на ВСЕ ТРИ провайдера обеспечивает reader/turkey_bot_test/conversation.py,
...
class LiveKgmCheck:
    ...
    reader/turkey_bot_test/avrasya/live_session_registry.py::LiveAvrasyaCheck —
    тот же принцип)."""
    ...
    # reader/turkey_bot_test/toll_check_repository.py), ни на что другое не
    # влияет.
...
class LiveKgmSessionRegistry:
    ...
    async def close_all(self) -> None:
        ...
        reader/turkey_bot_test/main.py::run(), finally) — безопасно вызывать
