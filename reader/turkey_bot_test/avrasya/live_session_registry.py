...
reader/turkey_bot_test/live_session_registry.py::LiveGibSessionRegistry) реестр
живых Avrasya-проверок по chat_id.

Один httpx.AsyncClient — одна in-flight проверка (см.
reader/turkey_bot_test/avrasya/session.py::AvrasyaSession) — здесь же
...
into GIB-specific abstractions... reader/turkey_bot_test/gib/* остаётся
...
провайдера (GIB и Avrasya) обеспечивает reader/turkey_bot_test/conversation.py,
...
class LiveAvrasyaCheck:
    ...
    # (см. reader/turkey_bot_test/toll_check_repository.py), ни на что другое
    # не влияет.
