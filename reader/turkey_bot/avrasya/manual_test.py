...
оборачивать автотестами, не вызывать из reader/turkey_bot/main.py.
...
HTTP 404 (долга нет) — см. reader/turkey_bot/avrasya/parser.py: "не
...
Использование:
    python -m reader.turkey_bot.avrasya.manual_test A123AA123

Плата нормализуется ЧЕРЕЗ существующий reader/turkey_bot/validation.py::
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python -m reader.turkey_bot.avrasya.manual_test <PLATE>", file=sys.stderr)
        sys.exit(2)
