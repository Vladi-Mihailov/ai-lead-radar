...
Полностью независимо от reader/turkey_bot/gib/* и
reader/turkey_bot/avrasya/* (см. задачу: "Не использовать существующий
reader/turkey_bot/avrasya parser для HTML KGM... Это два независимых
...
from reader.turkey_bot.kgm.parser import parse_delta_response
...
class KgmTransportError(Exception):
    ...
    reader/turkey_bot/kgm/models.py::KgmSubmitOutcome) — тот разбирает
...
class KgmSession:
    ...
    одного номера (см. reader/turkey_bot/avrasya/session.py::
    ...
    async def start(self) -> KgmCaptchaChallenge:
        ...
        той же сессии (см. reader/turkey_bot/avrasya/session.py::
    ...
    async def submit(self, *, plate: str, captcha_code: str) -> str:
        ...
        делает ИСКЛЮЧИТЕЛЬНО reader/turkey_bot/kgm/parser.py (тот же
