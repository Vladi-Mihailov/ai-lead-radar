...
reader/turkey_bot/gib/session.py::GibSession) — здесь же ассоциация
...
переживает рестарт — реестр стартует пустым. reader/turkey_bot/
...
class LiveGibCheck:
    ...
    # (см. reader/turkey_bot/check_repository.py), ни на что другое не влияет.
