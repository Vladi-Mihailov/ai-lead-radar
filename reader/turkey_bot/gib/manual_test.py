...
не оборачивать автотестами, не вызывать из reader/turkey_bot/main.py (его
...
with-mys-borc-list хотя бы один раз (см. reader/turkey_bot/gib/parser.py:
...
Использование:
    python -m reader.turkey_bot.gib.manual_test A123AA123
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python -m reader.turkey_bot.gib.manual_test <PLATE>", file=sys.stderr)
        sys.exit(2)
