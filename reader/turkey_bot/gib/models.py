...
class GibSubmitOutcome:
    ...
    kind:
      ...
      - "has_debt"    — успешный ответ, "data" непуст — ЧТО именно внутри,
        по конкретным полям сознательно НЕ парсится (см. models.py докстрок
        выше) — raw_data отдаётся как есть вызывающему коду;
