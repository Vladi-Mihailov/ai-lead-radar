...
class _FakeMessage:
    def __init__(self, message_id, sender_id, sender, get_sender_calls):
        self.id = message_id
...
def _make_messages(total, get_sender_calls, sender_for=None):
    ...
    for message_id in range(total, 0, -1):
        ...
        messages.append(_FakeMessage(message_id, sender_id, sender, get_sender_calls))
    return messages
...
async def test_fresh_group_completes_and_checkpoint_reflects_full_history(tmp_path):
    ...
    try:
        await sync_users_from_history(
            client, [Group(id=-100111, username=None, title="Test group")], repository, state_repository
        )
...
async def test_crash_mid_history_then_resume_completes_without_gaps_or_duplicates(tmp_path):
    ...
    try:
        ...
        await sync_users_from_history(crashing_client, [group], repository, state_repository)
        ...
        await sync_users_from_history(resuming_client, [group], repository, state_repository)
...
async def test_completed_group_is_skipped_without_reading_history_again(tmp_path):
    ...
    try:
        client = _FakeClient(entity, messages, get_sender_calls)
        await sync_users_from_history(client, [group], repository, state_repository)
        ...
        await sync_users_from_history(second_client, [group], repository, state_repository)
